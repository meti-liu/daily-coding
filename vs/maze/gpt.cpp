...
// 枚举用户的控制命令
enum CMD { CMD_QUIT = 1, CMD_UP = 2, CMD_DOWN = 4, CMD_LEFT = 8, CMD_RIGHT = 16, CMD_MARKRED = 32, CMD_MARKGREEN = 64, CMD_MARKYELLOW = 128, CMD_CLEARMARK = 256, CMD_AUTOMATIC = 512 };
...
void InitGame1();                            // 手动时初始化游戏数据
void InitGame2();                            // 自动时初始化游戏数据
void GetMazeSize();                        // 提示用户输入迷宫大小
void MakeMaze(int width, int height);    // 生成迷宫：初始化（注：宽高必须是奇数）
void Findroad(int x, int y);                // 生成正确路径并标记
...
bool CheckWin1();                            // 手动时检查是否到出口
bool CheckWin2();                            // 自动时检查是否到出口
bool Quit();                                // 询问用户是否退出游戏
void func1();                               // 用户选择手动进行游戏
void func2();                               // 用户选择自动进行游戏
...
void Welcome() {
    ...
    outtextxy(74, 436, _T("C：清除地图上的标记" "    "  "Q：放弃手动，直接显示出路"));
...
void InitImage() {
    ...
    SetWorkingImage();
    settextcolor(BLUE);
    TCHAR author[] = _T("Made by 刘建东 彭琳睿");
...
void InitGame1() {
    ...
    if (g_szMap.cx > g_szMap.cy) {
        ...
        g_szGPS.cy = 100;
        g_szGPS.cx = (int)(100.0 * g_szMap.cx / g_szMap.cy + 0.5);
    }
...
void Findroad(int x, int y) {
    if (f == 1) return;
    if (g_aryMap[x][y] == MAP_EXIT) f = 1;
    if (g_aryMap[x][y] == MAP_GROUND || g_aryMap[x][y] == MAP_ENTRANCE) {
        int t = g_aryMap[x][y];
        g_aryMap[x][y] = MAP_MARKRED;
        if (g_aryMap[x - 1][y] != MAP_MARKRED) Findroad(x - 1, y);
        if (g_aryMap[x + 1][y] != MAP_MARKRED) Findroad(x + 1, y);
        if (g_aryMap[x][y - 1] != MAP_MARKRED) Findroad(x, y - 1);
        if (g_aryMap[x][y + 1] != MAP_MARKRED) Findroad(x, y + 1);
        if (f == 1) return;
        g_aryMap[x][y] = t;
    }
...
int GetCmd() {
    ...
    if (GetAsyncKeyState('Q') & 0x8000)        c |= CMD_AUTOMATIC;
    return c;
}
...
void DispatchCmd(int cmd) {
    ...
    if (cmd & CMD_AUTOMATIC)    Findroad(1, 2);
}
...
bool CheckWin1() {
    if (g_ptPlayer.x == g_szMap.cx && g_ptPlayer.y == g_szMap.cy - 1) {
        HWND hwnd = GetHWnd();
        if (MessageBox(hwnd, _T("胜利!\n您想再来一局吗？"), _T("胜利victory"), MB_YESNO | MB_ICONQUESTION) == IDYES) {
            InitGame1();
            return false;
        }
        else
...
int main() {
    ...
    case 1:
        func1();
        break; // 添加 break 以防止继续执行 case 2
    case 2:
        func2();
        break;
    default:
        cout << "无效选择，请重新运行程序并选择有效的选项。" << endl;
    }
